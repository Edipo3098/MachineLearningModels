...
Whl = np.random.rand(D2,Number_weights_per_neurom)
...
Wol =   np.random.rand(Nol,Number_weights_per_neurom)
Outpu_ol = np.zeros(1)
Sensitividad_ol = np.zeros(((Nol,1) ))
Sensitividad2_ol = np.zeros(((Nol,1) ))
Senns_ol = np.zeros(((Nol,1) ))
dp_ol = np.zeros((Nol,Number_weights_per_neurom))   # delta weith
